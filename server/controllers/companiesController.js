...
export const register = async (req, res, next) => {
  ...
  if (!password) {
    next("Password must be greater than 6 is required!");
    return;
  }
  ...
  try {
    ...
    res.status(201).json({
      success: true,
      message: "Company Account creates Succesfully",
      user: {
        _id: company._id,
        name: company.name,
        email: company.email,
...
try{
  //validate
  if(!email || password){
    next("Plesae Provide User Credentials");
    return;
  }
...
export const updateCompanyProfile = async (req, res, next) =>{
  ...
  try{
    ...
    if(!name || !location || !about || !contact || !profileUrl){
      next("Please Provide all rtequirted fields");
      return;
    }
...
export const getCompanyProfile = async (req, res, next) =>{
  try {
    ...
    const comapny = await Companies.findById({_id: id});
    company.password = undefined;
...
export const getCompanies = async (req, res, next) =>{
  try {
    ...
    const limit = Number(req.quey.limit) || 20;
